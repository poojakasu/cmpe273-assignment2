...
import edu.sjsu.cmpe.library.dto.InputFrom;
...
public class BookResource {
    ...
    public Response getBookByIsbn(@PathParam("isbn") LongParam isbn) 
    {
		...
		return Response.status(500).entity(bookResponse).build();
    }
	    ...
	    @DefaultValue("available") @QueryParam("status") Status status) 
    {
		...
		if(("lost").equalsIgnoreCase(status.toString())){
			try{
				LibraryService.OrderForNewBook(isbn.get());
			}
			catch(JMSException e)
			{
				System.out.println("JMS error thrown while sending a message to the queue " + e.getMessage());
			}
		}
